...
const FluidPlane: FC<{ light?: boolean }> = ({ light = false }) => {
  ...
  return (
    <Plane args={[1, 1]} scale={[viewport.width, viewport.height, 1]}>
      <shaderMaterial
        ...
        fragmentShader={light ? fragmentShaderLight : fragmentShaderDark}
        transparent
      />
    </Plane>
  );
...
export const FluidOverlay: FC<{ light?: boolean }> = ({ light = false }) => {
  return (
    <FluidPlane light={light} />
  );
};
