...
export default function Home() {
  ...
  const rozsaRef = useRef<HTMLDivElement>(null);
  ...
  useEffect(() => {
    ...
    const updateScrollProgress = () => {
      if (!mainRef.current || !landingRef.current || !rozsaRef.current) {
      ...
      const rozsa = rozsaRef.current;

      const scrollTop = main.scrollTop;
      const landingTop = landing.offsetTop;
      const rozsaTop = rozsa.offsetTop;

      // Calculate progress: 0 = at landing page top, 1 = at ROZSA page top
      const totalDistance = rozsaTop - landingTop;
      const scrolledDistance = scrollTop - landingTop;

      // Clamp between 0 and 1, but keep at 1 if past the transition zone
      let progress = 0;
      if (scrollTop < landingTop) {
        progress = 0; // Before landing page
      } else if (scrollTop >= rozsaTop) {
        progress = 1; // Past ROZSA page - keep at final state
      } else {
  ...
  return (
    <main ref={mainRef} className='h-screen overflow-y-auto snap-y snap-mandatory'>
      ...
      >
        {/* Water Shader - Tilted back based on scroll, anchored at bottom */}
        <motion.div
          ...
          style={{
            ...
            transform: `perspective(2000px) rotateX(${scrollProgress * 0.05}deg) translateY(${scrollProgress * 0.05}%)`,
      ...
      <section ref={rozsaRef} className='h-screen w-full snap-start snap-always relative'>
        <ShaderScene />
      </section>
